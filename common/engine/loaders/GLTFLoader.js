...
export class GLTFLoader {
  ...
  async load(url) {
    ...
    if (this.gltfUrl.pathname.endsWith(".glb")) {
      await this.loadGLB(this.gltfUrl)
    } else {
  ...
  async loadGLB(url) {
    const arrayBuffer = await this.fetchBuffer(url);
    const dataView = new DataView(arrayBuffer);

    const jsonChunkLength = dataView.getUint32(12, true);

    const jsonChunk = new TextDecoder().decode(new Uint8Array(arrayBuffer, 20, jsonChunkLength));
    const json = JSON.parse(jsonChunk);

    const binaryChunkStart = 20 + jsonChunkLength
    const binaryChunkLength = dataView.byteLength - binaryChunkStart;
    const binaryData = new Uint8Array(arrayBuffer, binaryChunkStart, binaryChunkLength);
